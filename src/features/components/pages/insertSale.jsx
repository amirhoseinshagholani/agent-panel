...
const InsertSale = () => {
    ...
    const columns = useMemo(
        () => [
            ...
            //     id: 'delete',
            //     Cell: ({ row }) => (
            //         <button onClick={() => handleDelete(row.productList)}>حذف</button>
            //     ),
            // },
    ...
    return (
        <div>
            {/* <div className="mt-7 mr-3">
                <div className="text-2xl text-slate-600 mb-5">فرم ثبت سفارش </div>
                <div className="text-sm  text-slate-500">کاربر گرامی، این فرم جهت ثبت سفارش برای مشتریان در نظر گرفته شده است و صرفا فروش قطعی نمی باشد. پس از پرداخت مبلغ برای این سفارش فاکتور صادر خواهد شد</div>
            </div> */}
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="mb-3">
                                    ...
                                    <select id="selectedCustomer" {...register('customer')} onChange={handleTypeCustomer} className="h-10 text-gray-500 bg-slate-200 focus:outline-none w-full md:w-3/4 p-2 rounded rounded-lg" required>
                                        ...
                                        {/* {
                                    clues && clues.reverse().map(res => (
                                        <option key={res.id} value={[res.id,"clue"]}>{res.firstname} {res.lastname}</option>
                                    ))
                                } */}
                                        {
                                ...
                                {/* <div className="mb-3">
                            <textarea {...register('description')} rows={2} className="h-22 bg-slate-100 focus:outline-none w-3/4 p-2 rounded rounded-lg" type="text" placeholder="توضیحات" />
                        </div> */}
                            </div>
